    ...
    <div className="min-h-screen bg-[#f5f5f7] text-black font-sans pb-32">
      <div className="p-6 md:p-8 lg:p-16 max-w-4xl mx-auto">
        ...
        >
          ...
          <p className="text-xl md:text-2xl text-black/60 font-medium">Customize your Moments OS experience.</p>
        </motion.div>
        ...
        <motion.div
          ...
          className="bg-white rounded-[24px] md:rounded-[32px] p-6 md:p-8 lg:p-10 mb-8 md:mb-12 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.05)] border border-black/5 flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8"
        >
          {/* Avatar */}
          ...
          <div className="flex-1 text-center md:text-left w-full">
            ...
            <p className="text-black/50 font-medium mb-6 md:mb-8 text-sm md:text-base">{userProfile.email}</p>

            <div className="grid grid-cols-3 gap-2 md:gap-4 lg:gap-8 pt-6 border-t border-black/5">
              <div className="flex flex-col items-center md:items-start">
                <span className="text-xl md:text-2xl font-bold text-black mb-1">{userProfile.stats.photos}</span>
                <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-black/40">Photos</span>
              </div>
              <div className="flex flex-col items-center md:items-start">
                <span className="text-xl md:text-2xl font-bold text-black mb-1">{userProfile.stats.memories}</span>
                <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-black/40">Memories</span>
              </div>
              <div className="flex flex-col items-center md:items-start">
                <span className="text-xl md:text-2xl font-bold text-black mb-1">{userProfile.stats.storage}</span>
                <span className="text-[10px] md:text-xs font-bold uppercase tracking-wider text-black/40">iCloud Storage</span>
              </div>
            </div>
          </div>
        </motion.div>
        ...
        <div className="space-y-3 md:space-y-4 mb-16">
          <h3 className="text-lg md:text-xl font-bold text-black/40 tracking-tight ml-2 mb-4 md:mb-6">Preferences</h3>
          {settingsCategories.map((category, index) => {
            ...
            return (
              <motion.div
                ...
                whileTap={{ scale: 0.99 }}
                className="flex items-center gap-4 md:gap-6 bg-white/60 backdrop-blur-xl rounded-[24px] md:rounded-[28px] p-4 md:p-5 border border-white/80 cursor-pointer group shadow-sm hover:shadow-[0_10px_30px_rgba(0,0,0,0.06)] transition-all duration-300"
              >
                {/* Icon */}
                <div
                ...
                <div className="flex-1">
                  <h3 className="text-lg md:text-xl font-bold tracking-tight text-black mb-0.5">
                    {category.title}
                  </h3>
                  <p className="text-black/50 font-medium text-xs md:text-sm">{category.description}</p>
                </div>

                {/* Arrow */}
                <div className="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center bg-transparent group-hover:bg-[#f5f5f7] transition-colors duration-300">
                  <ChevronRight className="w-4 h-4 md:w-5 md:h-5 text-black/30 group-hover:text-black/80 transition-colors" />
                </div>
              </motion.div>
            );
          })}
